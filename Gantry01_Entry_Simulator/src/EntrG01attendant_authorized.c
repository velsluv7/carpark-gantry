...
int
EntrG01attendant_authorized( PtWidget_t *widget, ApInfo_t *apinfo, PtCallbackInfo_t *cbinfo )

	{
	message_id_013[0] = 0x0D;
	message_id_013[1] = 0x10;
	...
	if ((EntrG01_coid = name_open(ATTACH_POINT, 0)) == -1) {
		...
		if (MsgSend (EntrG01_coid,message_id_013,strlen (message_id_013)+1,rmsg,sizeof (rmsg)) == -1)
					{
